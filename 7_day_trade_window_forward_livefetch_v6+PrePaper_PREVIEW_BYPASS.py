...
def main():
    ...
    if run_mode == "AUTO_CYCLE":
        ...
        # -------------------------------------------------------------------
        # PREPAPER bypass preview (run even if NO TRADE)
        # -------------------------------------------------------------------
        ans = input("Bypass PREPAPER gates (preview TRADE survivors anyway)? (y/N): ").strip().lower()
        BYPASS_PREPAPER_GATE = ans in {"y", "yes", "1", "true", "on"}
        print(f"[CONFIG] BYPASS_PREPAPER_GATE={BYPASS_PREPAPER_GATE}", flush=True)
        ...
        print("[DEBUG] best_per_candidate count:", len(best_per_candidate))
        if best_per_candidate:
            print("[DEBUG] best_per_candidate[0] keys:", list(best_per_candidate[0].keys()))
            print("[DEBUG] best_per_candidate[0]:", best_per_candidate[0])
        ...
        if not winner:
            ...
            if not BYPASS_PREPAPER_GATE:
                return

            # BYPASS PREPAPER: run for each TRADE survivor in best_per_candidate
            print_section("PREPAPER (BYPASS PREVIEW)")
            ...
            out_pre = os.path.join(OUT_DIR, f"forwardtest_PREPAPER_7d_BYPASS_{ident_pre}_{pair}.xlsx")

            bypass_rows = []

            for cand in best_per_candidate:
                sc = str(cand.get("scenario", "")).strip().upper()
                if not sc:
                    raise KeyError(f"[BYPASS] Candidate missing 'scenario'. Keys={list(cand.keys())}")

                f_params = params_by_scen[sc]
                k_, t_, x_ = get_exit_params_from_finalist(f_params)

                print(f"\nPREPAPER WINDOW (UTC): {pre_start} -> {pre_end} | Pair={pair} | Scenario={sc}")
                ...
                bypass_rows.append(res_pre["summary_baseline"])
                bypass_rows.append(res_pre["summary_barrier"])

            summary_pre = pd.DataFrame(bypass_rows)
            print("PREPAPER SUMMARY (BYPASS PREVIEW)")
            ...
            print(f"\nSaved PrePaper BYPASS workbook: {out_pre}")
            return
        ...
        # ==========================================================================================
        # PREPAPER PREVIEW (BYPASS): run PREPAPER for TRADE-evaluated candidates even if no winner
        # ==========================================================================================
        if (winner is None) and BYPASS_PREPAPER_GATE:
            print_section("PREPAPER (BYPASS PREVIEW)")

            if not best_per_candidate:
                print("[BYPASS] No TRADE-evaluated candidates available to preview.")
                return

            print(f"[BYPASS] Previewing {len(best_per_candidate)} TRADE-evaluated candidates in PREPAPER window.")
            print(f"[BYPASS] PREPAPER WINDOW (UTC): {pre_start} -> {pre_end} | Pair={pair}")
            ...
            for cand in best_per_candidate:
                ...
                # Lookup k/t/x_bars from your finalist params (same source as winner path)
                f_params = params_by_scen[sc]
                k_, t_, x_ = get_exit_params_from_finalist(f_params)

                print("\n" + "-" * 100)
                print(f"PREPAPER WINDOW (UTC): {pre_start} -> {pre_end} | Pair={pair} | Scenario={sc}")
